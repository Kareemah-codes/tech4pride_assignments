...
Fetch API is a javascript Web API used to process requests and responses to servers. It enables developers to work with Web APIs.
Previously, XMLHttpRequest was used however those were callback based and may eventually lead to callback hell. Hence, Fetch API is a modern solution and it promise based.
...
async function getData(){
    ...
    try{
        const response = await fetch("https://jsonplaceholder.typicode.com/todos/1") // retrieving the first to do
        const data = await response.json() // changed the response to json
